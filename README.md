...
現時点の目的は「ゲームと外部AIプロセスを接続し、LLMまたはルールベースで合法手から行動を選ばせる」ことです。ゲームへ送るコマンドは必ずコード側で生成した合法手に限定し、LLM判断失敗時はルールベース判断へフォールバックします。
...
通常はOpenAI Responses APIで判断します。現在の起動設定は `gpt-5.4-mini` です。`OPENAI_API_KEY` が未設定、またはAPI呼び出しに失敗した場合は高速なルールベース判断へフォールバックします。`--use-codex` を付けた場合は、比較用にCodex CLI経由でも同じ合法手選択を試せます。
